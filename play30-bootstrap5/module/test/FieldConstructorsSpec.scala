...
class FieldConstructorsSpec extends Specification {
  ...
  def testFielConstructor(fcDefault: b5FieldConstructor, fcWithCustomFields: b5FieldConstructor, fcWithFeedbackTooltip: b5FieldConstructor) = {
    ...
    def simpleInputWithArgs(args: (Symbol, Any)*)(implicit fc: b5FieldConstructor) = b5.text(fooField, args*).body
    def simpleInputWithError(args: (Symbol, Any)*)(implicit fc: b5FieldConstructor) = b5.text(fooForm.withError("foo", "test-error-0").withError("foo", "test-error-1")("foo"), args*).body
    ...
    "render validation states" in {
      ...
      def testStatus(status: String, args: (Symbol, Any)*) = {
        val test = clean(simpleInputWithArgs(args*))
    ...
    "allow to set all its helpers as bootstrap custom fields" in {
      val customFile1 = b5.file(fooField, Symbol("_custom") -> true).body.trim
      val customFile2 = b5.file(fooField)(fcWithCustomFields, msgsProv).body.trim
      customFile1 must be equalTo customFile2

      val boolField = Form(single("foo" -> Forms.boolean))("foo")
      val customCheckbox1 = b5.checkbox(boolField, Symbol("_custom") -> true, Symbol("_text") -> "theText").body.trim
      val customCheckbox2 = b5.checkbox(boolField, Symbol("_text") -> "theText")(fcWithCustomFields, msgsProv).body.trim
      customCheckbox1 must be equalTo customCheckbox2

      val fruits = Seq("A" -> "Apples", "P" -> "Pears", "B" -> "Bananas")
      val customRadio1 = b5.radio(fooField, fruits, Symbol("_custom") -> true).body.trim
      val customRadio2 = b5.radio(fooField, fruits)(fcWithCustomFields, msgsProv).body.trim
      customRadio1 must be equalTo customRadio2

      val customSelect1 = b5.select(fooField, fruits, Symbol("_custom") -> true).body.trim
      val customSelect2 = b5.select(fooField, fruits)(fcWithCustomFields, msgsProv).body.trim
  ...
  "horizontal field constructor" should {
    ...
    val fcWithCustomFields = new b5.horizontal.HorizontalFieldConstructor(colLabel, colInput, isCustom = true)
  ...
  "vertical field constructor" should {
    ...
    val fcWithCustomFields = new b5.vertical.VerticalFieldConstructor(isCustom = true)
  ...
  "inline field constructor" should {
    ...
    val fcWithCustomFields = new b5.inline.InlineFieldConstructor(isCustom = true)
