...
class HelpersSpec extends Specification {
  ...
  def sampleInputTypeBody(theType: String) = b5.inputType(theType, fooField, sampleArgs*).body.trim

  "@text" should {
    "be equivalent to inputType with text type" in {
      b5.text(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("text")
    }
  }
  "@password" should {
    "be equivalent to inputType with password type" in {
      b5.password(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("password")
    }
    "not display its value" in {
      b5.password(fooFieldFilled("barValue"), sampleArgs*).body.trim must be equalTo sampleInputTypeBody("password")
    }
  }
  "@file" should {
    "be equivalent to inputType with file type" in {
      val file = b5.file(fooField, sampleArgs*).body.trim
    ...
    "render custom file properly" in {
      val customFile = b5.file(fooField, ((Symbol("_custom") -> true) +: sampleArgs)*).body.trim
  ...
  "@checkbox" should {
    ...
    "render custom checkbox properly" in {
      val body = clean(b5.checkbox(boolField, Symbol("_custom") -> true, Symbol("_text") -> "theText").body)
  ...
  "@radio" should {
    ...
    "render custom radio properly" in {
      val body = clean(b5.radio(fooField, fruits, Symbol("_custom") -> true).body)
  ...
  "@select" should {
    ...
    "add form-control class as default" in {
      b5.select(fooField, fruits).body must contain("class=\"form-control\"")
    }

    "allow setting additional classes" in {
      b5.select(fooField, fruits, Symbol("class") -> "extra_class").body must contain("class=\"form-control extra_class\"")
    }
    ...
    "render custom select properly" in {
      val body = b5.select(fooField, fruits, Symbol("_custom") -> true).body
  ...
  "@color" should {
    "be equivalent to inputType with date type" in {
      b5.color(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("color")
    }
  }
  "@date" should {
    "be equivalent to inputType with date type" in {
      b5.date(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("date")
    }
  }
  "@datetime" should {
    "be equivalent to inputType with date type" in {
      b5.datetime(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("datetime")
    }
  }
  "@datetimeLocal" should {
    "be equivalent to inputType with date type" in {
      b5.datetimeLocal(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("datetime-local")
    }
  }
  "@email" should {
    "be equivalent to inputType with email type" in {
      b5.email(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("email")
    }
  }
  "@month" should {
    "be equivalent to inputType with date type" in {
      b5.month(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("month")
    }
  }
  "@number" should {
    "be equivalent to inputType with date type" in {
      b5.number(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("number")
    }
  }
  "@range" should {
    "be equivalent to inputType with date type" in {
      b5.range(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("range")
    }
  }
  "@search" should {
    "be equivalent to inputType with date type" in {
      b5.search(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("search")
    }
  }
  "@tel" should {
    "be equivalent to inputType with date type" in {
      b5.tel(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("tel")
    }
  }
  "@time" should {
    "be equivalent to inputType with date type" in {
      b5.time(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("time")
    }
  }
  "@url" should {
    "be equivalent to inputType with date type" in {
      b5.url(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("url")
    }
  }
  "@week" should {
    "be equivalent to inputType with date type" in {
      b5.week(fooField, sampleArgs*).body.trim must be equalTo sampleInputTypeBody("week")
    }
  }
  ...
  "@formGroup" should {
    ...
    def testFormGroup(args: (Symbol, Any)*)(fc: b5.b5FieldConstructor, msgsProv: MessagesProvider) =
      clean(b5.freeFormGroup(args)(innerArgs => Html("<content>"))(using fc, msgsProv).body)
  ...
  "@static" should {
    ...
    "render with form-control-static class as default" in {
      b5.static("theLabel")(Html("theText"))(vfc, msgsProv).body must contain("<p class=\"form-control-static\">theText</p>")
    }

    "allow setting additional classes" in {
      b5.static("theLabel", Symbol("class") -> "extra_class")(Html("theText"))(vfc, msgsProv).body must contain("<p class=\"form-control-static extra_class\">theText</p>")
    }
  ...
  "@buttonType" should {
    ...
    def buttonTypeBody(args: (Symbol, Any)*) = b5.buttonType(sampleType, args*)(Html(sampleContent))(vfc, msgsProv).body
  ...
  def sampleButtonTypeBody(theType: String) = b5.buttonType(theType, sampleArgs*)(Html("content"))(vfc, msgsProv).body.trim

  "@submit" should {
    "be equivalent to buttonType with submit type" in {
      b5.submit(sampleArgs*)(Html("content"))(vfc, msgsProv).body.trim must be equalTo sampleButtonTypeBody("submit")
    }
  }
  "@reset" should {
    "be equivalent to buttonType with reset type" in {
      b5.reset(sampleArgs*)(Html("content"))(vfc, msgsProv).body.trim must be equalTo sampleButtonTypeBody("reset")
    }
  }
  "@button" should {
    "be equivalent to buttonType with button type" in {
      b5.button(sampleArgs*)(Html("content"))(vfc, msgsProv).body.trim must be equalTo sampleButtonTypeBody("button")
    }
  }
  ...
  "@multifield" should {
    ...
    def multifield(form: Form[(String, String)], globalArgs: Seq[(Symbol, Any)] = Seq(), fieldsArgs: Seq[(Symbol, Any)] = Seq())(fc: b5.b5FieldConstructor, msgsProv: MessagesProvider) =
      clean(b5.multifield(form("foo"), form("bar"))(globalArgs, fieldsArgs)(cfc => Html(testInputsString))(using fc, msgsProv).body)
    ...
    "render validation states" in {
      ...
      def testStatus(status: String, withFieldsArgs: Boolean, args: (Symbol, Any)*) = {
        val test = if (withFieldsArgs)
          clean(b5.multifield(fooForm("foo"))(globalArgs = Seq(), fieldsArgs = args)(cfc => b5.text(fooForm("foo"), args*))(vfc, msgsProv).body)
        else
          clean(b5.multifield(fooForm("foo"))(globalArgs = args, fieldsArgs = Seq())(cfc => b5.text(fooForm("foo"), args*))(vfc, msgsProv).body)
